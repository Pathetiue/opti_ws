...
def receiveRigidBodyFrame(id, pos, rotation):
    ...
    msg.euler.x = angle[0]/3.14*180
    msg.euler.y = angle[1]/3.14*180
    msg.euler.z = angle[2]/3.14*180
    s1.acquire()
...
def pub():
    while not rospy.is_shutdown():
        ...
        for i in range(total_rigidBody):
            ...
            print('v_x:',l_odom[i][l_index[i]].linear.x,'v_y',l_odom[i][l_index[i]].linear.y)  
            print('x:',l_odom[i][l_index[i]].position.x ,'y',l_odom[i][l_index[i]].position.y, 'z', l_odom[i][l_index[i]].position.z)
            print('pitch:',l_odom[i][l_index[i]].euler.x ,'roll',l_odom[i][l_index[i]].euler.y, 'yaw', l_odom[i][l_index[i]].euler.z)
            print('---')
...
if __name__ == '__main__':
    ...
    streamingClient = NatNetClient("192.168.1.113")  #Net2
